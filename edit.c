...
void edit(char *filename, char *tag_type, char *new_tag)
{
    char *tag_name = NULL;
    // check tag type
    ...
    FILE *fp,*fp1;
    // open sample.mp3 in read mode
    fp = fopen(filename,"r");
    // open temp.mp3 in write mode
    ...
    for(int i = 0; i < 6; i++)
    {
        ...
        // read tag from sample.mp3 file
        fread(tag,sizeof(char),4,fp);
        tag[4] = '\0';
        // read tag size from sample.mp3 file
        fread(&size,sizeof(int),1,fp);
        // convert big to little endian
        endian_conversion(&size, sizeof(size));
        // read skip 3 bytes from sample.mp3 file
        fread(skip_byte, sizeof(char),3,fp);
        // allocate dynamic memory
        char *content = malloc(size+1);
        // read tag content from sample.mp3 (size - 1 because we skipped for \0)
        fread(content,sizeof(char), size-1,fp);
        content[size-1] = '\0';
        // check entered tag_type and sample.mp3 tag matching or not
        if(strcmp(tag_name,tag) == 0)
        {
            ...
            // copy new_tag content size to temp.mp3
            fwrite(&new_size,sizeof(int),1,fp1);
            // copy skip 3 bytes to temp,mp3(1 for \0, 2 for flag)
            fwrite(skip_byte,sizeof(char),3,fp1);
            // convert little to big endian
            endian_conversion(&new_size,sizeof(new_size));
            // copyt new_tag content to temp.mp3 file
